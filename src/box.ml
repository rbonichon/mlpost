...
let empty ?(width = Num.zero) ?(height = Num.zero) ?style ?name ?brush
  ...
  mkbox ?style ?name ~dx:zero ~dy:zero ?brush ~stroke ?pen ?dash ?fill width
    height Point.origin Emp

let empty_from_box ?style ?name ?brush ?(stroke = None) ?pen ?dash ?fill box =
  mkbox ?style ?name ?brush ~stroke ?pen ?dash ?fill (width box) (height box)
    (ctr box)
    (Grp ([||], merge_maps [ box ]))
